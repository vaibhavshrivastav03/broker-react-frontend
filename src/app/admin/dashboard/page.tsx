...
import {
  ...
  Key, Activity, Star, Search, Copy, Check,PlusCircle,DollarSign, Upload,
...
/* 🔥 ADD BROKER STATE */
  ...
  async function handleSubmitListing() {
   ...
   // ✅ ONLY ON CREATE
    ...
    try {
      if (editingId) {
        await api.put(`/admin/update-listings/${editingId}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Listing updated");
      } else {
        await api.post("/admin/create-listings", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  try {
    const res = await api.patch(`/admin/vessels/${id}/feature`, {
      is_featured: !current,
    });
    ...
    setBoats((prev) =>
      prev.map((b) =>
        b.id === id
          ? { ...b, is_featured: res.data.is_featured }
          : b
      )
    );
  } catch (err) {
